 ...
 * @hide
 */
class FeedsMaterialProgressDrawable extends Drawable implements Animatable {
    ...
    public FeedsMaterialProgressDrawable(Context context, View parent) {
        mParent = parent;
     ...
     *
     * @param size One of {@link FeedsMaterialProgressDrawable.LARGE} or
     *             {@link FeedsMaterialProgressDrawable.DEFAULT}
     */
...
//            Log.i("draw()", "(" + mStartTrim + ", " + mEndTrim + ", " + sweepAngle + ", " + mRotation + "), (" + startAngle + ", " + endAngle + ")");
            ...
            SweepGradient mSweepGradient = new SweepGradient(c.getWidth() / 2,
                    c.getHeight() / 2, //以圆弧中心作为扫描渲染的中心以便实现需要的效果
                    mGradientColor, //这是我定义好的颜色数组，包含2个颜色：#35C3D7、#2894DD
                    new float[]{0, Math.abs(mStartTrim - mEndTrim)});   // 必须从比较小的值开始（非单调的话会有奇怪的结果），其实就是整个颜色变化的区域（相对于整个圆，如为 null，就是一整圈以后颜色渐变完结）
            // 旋转渐变使之同 ring 的初始角度吻合
            Matrix matrix = new Matrix();
            float realHeaderAngle;
            if (mStartTrim < mEndTrim) {
                realHeaderAngle = startAngle;
            } else {
                realHeaderAngle = endAngle;
            }
            matrix.setRotate(realHeaderAngle - 10, c.getWidth() / 2, c.getHeight() / 2);
...
//            mPaint.setColor(mCurrentColor);
            ...
            drawRoundHeader(c, realHeaderAngle + Math.abs(sweepAngle), arcBounds);
        ...
        private void drawRoundHeader(Canvas c, float angle, RectF bounds) {
            if (mShowArrow) {
             return;
            }
            mArrowPaint.setColor(mCurrentColor);
            float radius = mStrokeWidth / 2f;
            float x = (float) (mRingCenterRadius * Math.cos(0) + bounds.centerX());
            float y = (float) (mRingCenterRadius * Math.sin(0) + bounds.centerY());
            c.rotate(angle, bounds.centerX(), bounds.centerY());
            c.drawCircle(x, y + radius, // 圆心
                    radius,  //半径
                    mArrowPaint);
        }
...
//            mCurrentColor = color;
            mCurrentColor = mGradientColor[1];
        }
